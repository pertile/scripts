...
auth = HttpNtlmAuth('fiduciaria\\fpertile', 'laBitacora8')
root_url = "http://sp01/impuestos"
site = Site('http://sp01/impuestos', auth=auth)

sp_list = site.List('Requerimientos')

list_data = sp_list.GetListItems()

for x in list_data:
    urls = sp_list.GetAttachmentCollection(x['ID'])
    for url in urls:
        name = url.split("/")[-1]
        
        response = requests.get(url)
        open("c:\\temp\\reqimpuestos\\" + name, "wb").write(response.content)
...
# for x in f:
#   nombreArchivo = "\\\\sp01\\impuestos\\facturas\\" + x[x.rfind("/")+1:]
